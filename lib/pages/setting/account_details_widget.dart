  ...
  Future<void> _loadAccountData() async {
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('فشل في تحميل البيانات: $e')));
      }
    }
  }
    ...
    try {
      ...
      if (newEmail != oldEmail) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('تم إرسال رسالة تحقق إلى البريد الإلكتروني الجديد')),
        );
        ...
        if (verified) {
          ...
          await FirebaseFirestore.instance.collection('users').doc(user.uid).set(
            {'email': newEmail, 'account_status': 'active'},
            SetOptions(merge: true),
          );

          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('تم التحقق من البريد الإلكتروني وحفظ التعديلات')),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
        ...
        appBar: AppBar(
          ...
          leading: Padding(
            ...
            child: Container(
              ...
              child: IconButton(
                icon: Icon(Icons.arrow_back_ios_new_rounded, color: theme.primaryText, size: 18),
          ...
          child: Center(
            child: SingleChildScrollView(
              child: Container(
                ...
                decoration: BoxDecoration(color: theme.containers, borderRadius: BorderRadius.circular(20)),
                ...
                child: Column(
                  children: [
                    SizedBox(
                      ...
                      child: Stack(
                        children: [
                          Center(child: Text('معلومات الحساب', style: theme.titleSmall)),
                          Positioned(
                            left: 0,
                            top: 0,
                            bottom: 0,
                            child: Container(
                              width: 40,
                              height: 40,
                              decoration: BoxDecoration(color: const Color(0xB1E1A948), borderRadius: BorderRadius.circular(8)),
                              child: IconButton(
                    ...
                    if (_model.isEditing)
                      FFButtonWidget(
                        ...
                        options: FFButtonOptions(
                          height: 44,
                          width: double.infinity,
                          color: _model.isLoading ? theme.secondaryButtonsOnLightBackgroundsNavigationBar : theme.iconsOnLightBackgroundsMainButtonsOnLightBackgrounds,
                          textStyle: theme.titleSmall.copyWith(color: theme.containers, fontSize: 16),
                          elevation: 2,
...
class _TextFieldBox extends StatelessWidget {
  ...
  const _TextFieldBox({required this.controller, this.focusNode, required this.enabled, this.keyboardType, this.textInputAction, this.hint});

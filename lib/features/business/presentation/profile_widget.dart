...
class BusinessProfileWidgetState extends State<BusinessProfileScreen> {
      ...
      setState(() {
        ...
        if (widget.uid != null) {
          _campaignList = _campaignList.where((c) {
            ...
            final endDate =
                c['end_date'] is Timestamp ? (c['end_date'] as Timestamp).toDate() : c['end_date'] as DateTime?;
  ...
  List<Widget> _buildCampaignPlatforms(Map<String, dynamic> e) {
    ...
    return platformNames.map((platformNameStr) {
      ...
      return Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Text(platformObj.nameAr, style: const TextStyle(color: Colors.black87, fontSize: 13)),
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                                      ...
                                      children: [
                                        ...
                                        if (widget.campaignId == null) ...[
                                          ...
                                          if ((_profileData?.description ?? '').isNotEmpty) ...[
                                            ...
                                            Align(
                                              ...
                                              child: Text(
                                                ...
                                                textAlign: TextAlign.right,
                                                style:
                                                    theme.bodyMedium.copyWith(height: 1.6, color: theme.secondaryText),
                                              ),
                                            ),
                                            const SizedBox(height: 24),
                                          ],
                                          ...
                                          if (widget.uid == null)
                                            FFButtonWidget(
                                              onPressed: () =>
                                                  context.pushNamed(BusinessProfileFormWidget.routeNameEdit),
                            ...
                            Container(
                              width: double.infinity,
                              padding: (widget.campaignId == null) ? const EdgeInsets.all(16) : const EdgeInsets.all(0),
                              decoration: BoxDecoration(
  ...
  Future<void> _loadMyApplications() async {
    ...
    try {
      ...
      final appsSnap = await firebaseFirestore.collection('applications').where('influencer_id', isEqualTo: myId).get();
      final appliedIds =
          appsSnap.docs.map((d) => (d.data()['campaign_id'] as String? ?? '')).where((id) => id.isNotEmpty).toSet();
      ...
      final offersSnap = await firebaseFirestore
          ...
          .get();
      final offeredIds =
          offersSnap.docs.map((d) => (d.data()['campaign_id'] as String? ?? '')).where((id) => id.isNotEmpty).toSet();
  ...
  Future<void> _applyToCampaign(Map<String, dynamic> campaign) async {
    ...
    final campaignId = campaign['campaign_id'] as String? ?? campaign['id'] as String? ?? '';
    ...
    if (_appliedCampaignIds.contains(campaignId)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('لقد تقدمت على هذه الحملة مسبقاً', textDirection: TextDirection.rtl),
        ),
      );
      return;
    }
    ...
    try {
      final profileSnap =
          await firebaseFirestore.collection('profiles').where('profile_id', isEqualTo: myId).limit(1).get();
      ...
      if (profileSnap.docs.isNotEmpty) {
        ...
        if (raw.isNotEmpty) {
          influencerImageUrl = raw.contains('?') ? '${raw.split('?').first}?alt=media' : '$raw?alt=media';
        }
      }
    } catch (_) {}
          ...
          child: AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                Text(
                  '"${campaign['title'] ?? ''}"',
                  style: t.titleSmall.copyWith(fontWeight: FontWeight.w700, color: t.primary),
                  textAlign: TextAlign.end,
                ),
                const SizedBox(height: 8),
                Text(
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('تم إرسال تقديمك بنجاح 🎉', textDirection: TextDirection.rtl),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('حدث خطأ أثناء التقديم: $e', textDirection: TextDirection.rtl),
        ),
      );
    }
  }
  ...
  Widget _tileCampaignSpecial(Map<String, dynamic> e) {
    ...
    final DateTime? endDate =
        e['end_date'] is Timestamp ? (e['end_date'] as Timestamp).toDate() : e['end_date'] as DateTime?;
    ...
    final String campaignId = (e['campaign_id'] as String? ?? e['id'] as String? ?? '');
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(title, style: valueStyle.copyWith(color: theme.primaryText), textAlign: TextAlign.end),
                    const SizedBox(height: 8),
                    if (s.isNotEmpty || en.isNotEmpty) ...[
                      Text('الفترة الزمنية', style: labelStyle, textAlign: TextAlign.end),
                      Text('من $s إلى $en',
                          style: valueStyle.copyWith(color: theme.secondaryText), textAlign: TextAlign.end),
                      const SizedBox(height: 8),
                    ],
                    Text('تفاصيل الحملة', style: labelStyle, textAlign: TextAlign.end),
                    Text(description, style: valueStyle.copyWith(color: theme.secondaryText), textAlign: TextAlign.end),
                    const SizedBox(height: 8),
